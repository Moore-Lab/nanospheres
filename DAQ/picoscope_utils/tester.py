...
bufferMax = np.zeros(shape=(2,sizeOfOneBuffer), dtype=np.int16)
...
status["setDataBuffersA"] = ps.ps4000aSetDataBuffers(chandle,
                                                     ps.PS4000A_CHANNEL['PS4000A_CHANNEL_A'],
                                                     bufferMax[0].ctypes.data_as(ctypes.POINTER(ctypes.c_int16)),
                                                     None,
                                                     sizeOfOneBuffer,
                                                     memory_segment,
...
status["setDataBuffersB"] = ps.ps4000aSetDataBuffers(chandle,
                                                     ps.PS4000A_CHANNEL['PS4000A_CHANNEL_B'],
                                                     bufferMax[1].ctypes.data_as(ctypes.POINTER(ctypes.c_int16)),
                                                     None,
                                                     sizeOfOneBuffer,
                                                     memory_segment,
                                                     ps.PS4000A_RATIO_MODE['PS4000A_RATIO_MODE_NONE'])
...
bufferComplete = np.zeros(shape=(2,totalSamples), dtype=np.int16)
...
adc2mVChAMax = adc2mV(bufferComplete[0], channel_range, maxADC)
adc2mVChBMax = adc2mV(bufferComplete[1], channel_range, maxADC)
